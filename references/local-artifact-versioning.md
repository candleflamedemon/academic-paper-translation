...
4. Within that version subfolder, create a separate paper folder for each translated paper and put that paper's deliverables, exported files, and generated reports there. If the user explicitly asks to modify an existing project file in place, modify that existing file at its original path.
...
## Paper Folder and Deliverable Naming

Before writing the translation package, identify the English paper title displayed in the paper itself and translate it faithfully into natural Chinese. If embedded PDF metadata conflicts with the displayed title, prefer the displayed title. Identify the publication year of the version being translated from the paper's bibliographic citation or publisher record; for a preprint, use that version's posted year. Do not substitute the PDF's file creation or download year. If the publication year cannot be verified, ask the user rather than inventing one.

Use the same sanitized or shortened `中文标题译文` and four-digit `发布年份` in the paper folder and every default deliverable filename:

```text
任务成果/<版本目录>/<中文标题译文>_<发布年份>/
├── <中文标题译文>_<发布年份>_中文译文版.docx
├── <中文标题译文>_<发布年份>_中英对照版.docx
├── <中文标题译文>_<发布年份>_英文原文.pdf
└── <中文标题译文>_<发布年份>_校验报告.json   # only when a report is generated
```
...
- Prefer the complete translated title. If the title exceeds 80 characters or the nested folder and filenames risk the platform path limit, shorten it to a clear, distinctive title while preserving the paper's main subject and differentiating terms. Use the same shortened stem for the paper folder and every file in the package.
- Keep the edition suffixes exactly as shown so files remain distinguishable in search results and file explorers.
- An explicit user-supplied filename or naming scheme takes precedence. Otherwise, generic names such as `chinese.docx`, `bilingual.docx`, and `original_copy.pdf` are not acceptable defaults. In a batch, keep each paper's files in its own matching folder.
...
Before finishing, confirm that each paper's deliverables are together in its matching paper folder inside the new version folder, the update summary accurately describes this version, the folder names follow their conventions, previous versions were not modified, and the user is told the full path to the new artifact folder.
